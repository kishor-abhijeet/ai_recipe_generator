...
//   console.error(
//     "Warning : GEMINI_API_LEY is not set. AI features will not work.",
//   );
// }
...
export const generatePantrySuggestions = async (
  ...
  try {
    ...
    const expiringText =
      expiringItems.length > 0
        ? `Priority: ${expiringItems.join(", ")}`
        : "";

    const prompt = `Ingredients: ${ingredients}. ${expiringText}
Return 3 recipe ideas as JSON array.`;
...
Return 3 cooking tips as JSON array.`;
